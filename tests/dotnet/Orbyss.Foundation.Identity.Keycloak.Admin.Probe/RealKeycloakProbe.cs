...
using Orbyss.Foundation.Identity.Admin;
...
internal static class RealKeycloakProbe
{
    ...
    internal static async Task RunAsync()
    {
        ...
        string? scopeId = null;
        string? groupId = null;
        const string roleName = "orbyss-foundation-admin-probe-role";
        try
        {
            ...
            await access.CreateRoleAsync(roleName, "Disposable Orbyss Foundation acceptance role");
            ...
            groupId = await access.CreateGroupAsync("orbyss-foundation-admin-probe-group");
            await access.AddUserToGroupAsync(userId, groupId);
            await access.RemoveUserFromGroupAsync(userId, groupId);

            applicationId = await applications.CreateAsync(new("orbyss-foundation-admin-created-client", "Admin-created client", ServiceAccountsEnabled: true));
            await applications.UpdateAsync(applicationId, new("orbyss-foundation-admin-created-client", "Updated client", ServiceAccountsEnabled: true));
            ...
            scopeId = await scopes.CreateAsync("orbyss-foundation-admin-created-scope", "Disposable acceptance scope");
    ...
    private static ServiceProvider BuildProvider()
    {
        ...
        settings.ConfigurationData[$"{prefix}:Realm"] = "orbyss-foundation";
        settings.ConfigurationData[$"{prefix}:AdminRealm"] = "orbyss-foundation";
        settings.ConfigurationData[$"{prefix}:ClientId"] = "orbyss-foundation-admin-probe";
        ...
        new FoundationKeycloakUsersAdminFeature(settings).ConfigureServices(collection);
        new FoundationKeycloakApplicationsAdminFeature(settings).ConfigureServices(collection);
        new FoundationKeycloakScopesAdminFeature(settings).ConfigureServices(collection);
        new FoundationKeycloakAccessAdminFeature(settings).ConfigureServices(collection);
        new FoundationKeycloakEnrollmentAdminFeature(settings).ConfigureServices(collection);
        new FoundationKeycloakSessionsAdminFeature(settings).ConfigureServices(collection);
        new FoundationKeycloakRealmOperationsFeature(settings).ConfigureServices(collection);

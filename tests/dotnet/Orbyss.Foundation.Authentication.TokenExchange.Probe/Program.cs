...
static ServiceProvider BuildProvider(
    ...
    DateTimeOffset now)
{
    var settings = new ShellSettings(new ShellId("probe"), ["Orbyss.Foundation.Authentication.TokenExchange"]);
    ...
    new FoundationTokenExchangeFeature(settings).ConfigureServices(services);

...
static ServiceProvider CreateProvider(string environmentName, bool allowHttpForLocalDevelopment)
{
    ...
    services.AddSingleton<IOptions<FoundationWebOptions>>(Options.Create(new FoundationWebOptions
    {
        Authority = allowHttpForLocalDevelopment
            ? "http://localhost:8080/realms/orbyss-foundation"
            : "https://identity.example/realms/orbyss-foundation",
        ClientId = "orbyss-foundation-bff",
...
sealed class ProbeEnvironment(string environmentName) : IHostEnvironment
{
    ...
    public string ApplicationName { get; set; } = "Orbyss.Foundation.Authentication.BffCookie.Probe";

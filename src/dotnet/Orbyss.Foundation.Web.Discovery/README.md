...
Optional `IWebShellFeature`, identity `Orbyss.Foundation.Web.Discovery`. Add its NuGet package to the
...
The default catalogue reads `web/generated/orbyss-foundation/publication.json` below the content root.
Override through shell setting `Foundation:Web:Discovery:OutputDirectory`. Generate with

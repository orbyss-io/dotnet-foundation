...
internal sealed class DPoPOptionsValidator(IHostEnvironment environment) : IValidateOptions<DPoPOptions>
{
    ...
    public ValidateOptionsResult Validate(string? name, DPoPOptions options)
    {
        ...
        {
            failures.Add("Foundation:Authentication:DPoP:PublicOrigin must be an exact absolute origin.");
        }
        else
        {
            ...
            {
                failures.Add(
                    "Foundation:Authentication:DPoP:PublicOrigin must use HTTPS; local HTTP requires the explicit development override.");
            }
        }

        if (options.ProofMaxAgeSeconds is < 10 or > 300)
        {
            failures.Add("Foundation:Authentication:DPoP:ProofMaxAgeSeconds must be between 10 and 300.");
        }
        if (options.ClockSkewSeconds is < 0 or > 30)
        {
            failures.Add("Foundation:Authentication:DPoP:ClockSkewSeconds must be between 0 and 30.");
        }
        if (options.NonceLifetimeSeconds is < 10 or > 300)
        {
            failures.Add("Foundation:Authentication:DPoP:NonceLifetimeSeconds must be between 10 and 300.");
        }

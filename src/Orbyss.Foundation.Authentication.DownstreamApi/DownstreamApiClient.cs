    ...
    ICurrentAccessTokenAccessor currentAccessToken) : IDownstreamApiClient
{
    ...
    internal const string HttpClientName = "Orbyss.Foundation.Authentication.DownstreamApi";
        ...
        CancellationToken cancellationToken)
    {
        var provider = services.GetService<IClientCredentialsTokenProvider>()
            ?? throw new InvalidOperationException(
                "ClientCredentials mode requires the Orbyss.Foundation.Authentication.ClientCredentials feature in this shell.");
        ...
        CancellationToken cancellationToken)
    {
        var exchange = services.GetService<ITokenExchangeService>()
            ?? throw new InvalidOperationException(
                "TokenExchange mode requires the Orbyss.Foundation.Authentication.TokenExchange feature in this shell.");
    ...
    private static void ValidateRelativeRequest(HttpRequestMessage request)
    {
        ...
        {
            throw new ArgumentException("Downstream authorization is owned by Orbyss Foundation.", nameof(request));
        }
    }
}
